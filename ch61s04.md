...
```
BLCKSZ是一个常量，通常为8192。如果shared_buffers = 128M，则 NBuffers = (128 * 1024 * 1024) / (8 * 1024) = 16384。NBuffers的类型是int，是有符号整数，只能表示2G。如果它的数据类型是unsigned int，那么它可以表示4G。理论上shared pool的大小为2G X 8K = 16T。对于目前(2023年），高端服务器的配置不过1到2T的内存，所以拥有16T的内存的计算机还非常罕见。
