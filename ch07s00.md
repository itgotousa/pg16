...
AllocBlockData表示一个Block，AllocBlock是相应的指针。一个Block的结构分为两部分，头部的AllocBlockData和其后的数据区。再次强调一下：初始Block和其它Block不同，它的结构分为三部分：AllocSetContext，AllocBlockData和数据区。初始Block和非初始Block的区别可以用下图来表示：
