...
### 概述
我们可以一个PostgreSQL的内存池，理解为一堆内存块Block，这些Block是通过malloc()分配的，并且最终通过free()来进行释放，且它们组成一个双向链表。Block的大小有三个尺寸相关的变量规定，它们的单位均为字节(in bytes)：
- initBlockSize - 初始块的大小，通常初始块是8KB（8192个字节）。
- nextBlockSize - 下一次分配的Block的大小。
- maxBlockSize  - 最大Block的大小。

在PostgreSQL的缺省实现策略中，每当分配一个新的Block时，它的体积都比前一次分配的Block大一倍。例如：第一个初始块是8KB，则第二次分配的Block的大小是16KB，再下一次是32KB，再下一次是64KB，依次类推。这些Block组成一个双向链表，但不是循环链表，是有头有尾的，如下图所示：

![](d0042.svg)

第一次分配的初始块(Initial Block)和其后诞生的Block有一点不同：在它的头部有一个“控制头”，是C语言的一个结构体，它里面的成员变量用于控制整个内存池的分配销毁等行为。在PostgreSQL中这个控制头叫AllocSetContext，如上图中的右下角的大矩形，它实际上是初始块头部很小的一块区域，只是为了显示其细节，图中进行了放大。这个初始块也可以叫做“常驻块”，表示其上保存着一个AllocSetContext结构。正式因为初始块和其后诞生的Block有这个不同，所以产生了内存池的Delete和Reset两种操作的概念，具体含义稍后讨论。

从上图中我们可以AllocSetContext有一个指针blocks，指向内存块双向链表的头部的Block，我们可以把这个Block称为“头部Block”。头部Block是内存池中最新诞生的Block，它和初始块/常驻块不一定相同。头部Block永远是当前内存池中最大的，且只有在它里面可能存在用于未来分配的空闲空间。在它后面的Block，要么是已经没有空闲空间了，要么残存的空闲空间都被加入到了一个空闲空间列表freelist数组当中了。
