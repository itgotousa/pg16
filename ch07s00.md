...
当使用者使用palloc()函数向内存池申请内存时，内存池返回给使用者的是内存片(Chunk)，Chunk是从某一个Block中切下来的一小块。内存池寻找空闲Chunk时，只在活跃Block上的空闲内存区中寻找,只有活跃Block上的空闲内存不够了，才会再次malloc()出一个更大的新块。按照这种规律，我们可以看出，活跃Block后面的老Block中基本上没有什么空闲内存了。内存池只在活跃Block里面找空闲内存，不用遍历双向链表，这就是“活跃”的含义。上图中最左边的Block便是活跃Block。Block组成的双链只是用于内存池销毁或者重置的时候进行遍历使用。
...
内存池内部Block是malloc()和free()的基本单位，使用者向内存池申请的是内存片(Chunk)。内存池(Pool)，内存块(Block)和内存片(Chunk)是内存池系统中的三个基本概念。它们的关系总结如下：
- Pool包含一个或者多个Block。在内存池被创建后，只有一个初始Block。在初始Block的头部有一个AllocSetContext控制头。随着申请者越来越多，Pool再malloc()出更多的Block。
- 一个Block包含一个或者多个Chunk。内存池中的所有Block组成一个双向链表，其中第一个Block被叫做活跃Block。
- 任何一个Chunk只属于一个Block。
- 任何一个Block只属于一个Pool。
