...
为PostgreSQL设计的内存池管理子系统，提供了palloc()和pfree()的内存分配和释放接口函数，非常类似malloc()和free()。我们可以把PostgreSQL的内存池理解为一堆内存块(Block)的集合。这些Block是通过malloc()分配的，最终通过free()来进行释放，且它们组成一个双向链表。Block的大小有三个整型变量来控制，它们的单位均为字节(in bytes)：
