...
当申请者使用palloc()函数向内存池申请内存时，内存池返回给申请者的内存叫内存片(Chunk)，Chunk是从某一个Block中切下来的一小块。内存池为Chunk寻找空闲内存时，只在活跃Block上的空闲内存区中寻找。只有当活跃Block上的空闲内存不够时，才会再次malloc()出一个更大的新块。按照这种规律，我们可以看出，活跃Block后面的老Block中基本上没有什么空闲内存了。内存池只在活跃Block里面寻找空闲内存，不用遍历双向链表，这就是“活跃”的含义。上图中最左边的Block便是活跃Block。Block组成的双链只是用于内存池销毁或者重置的时候进行遍历使用。
