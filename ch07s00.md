...
- 任何一个Block只属于一个Pool。一个Block包含一个或者多个Chunk。只有Block才是通过malloc()和free()进行创建和释放的。
- Chunk返回给申请者的内存，它是某个Block中的空闲内存的一部分，所以任何一个Chunk只属于一个Block。
...
MemoryContext是指向一个结构体MemoryContextData的指针。PostgreSQL对数据结构命名的时候，喜欢用xxxxData表示真正的结构体，用xxxx表示指向该结构体的指针。在后面的讨论中，为了减少单词的长度，本书把结构体和指向该结构体的指针大致视为同一个概念，可能会交叉使用。后面也有相似的例子，不再赘述。
