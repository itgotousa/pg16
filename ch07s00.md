...
## 概述
一个PostgreSQL的内存池，实际上是一些内存块的双向链表结构。这些内存块的尺寸有三个变量规定：
- initBlockSize - 初始块的大小(单位是字节)
- nextBlockSize - 下一次分配的内存块的大小(单位是字节)
- maxBlockSize  - 最大内存块的大小(单位是字节)，它是这个内存池中内存块的最大尺寸。

每当分配一个新的内存块的时候，都比前一次分配的内存块大一倍。例如：第一个初始块是8KB，则第二次分配的内存块的大小是16KB，再下一次是32KB，再下一次是64KB，依次类推。这些内存块组成一个双向链表，但是不是循环链表，而是有头有尾，如下图所示：

![](d0042.svg)

第一次分配的初始块和其它内存块有一点不同，在它的头部有一个结构体叫做AllocSetContext，如上图中的右下角的大矩形，实际上是1号内存块头部很小的一块区域，只是为了显示其细节，我进行了放大。这个初始块也可以叫做“常驻块”，表示其上保存着一个AllocSetContext结构。

大家看到了，1号块AllocSetContext有一个指针blocks，指向内存块双向链表的头部。
