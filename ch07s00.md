...
C语言初学者都知道，在C程序中申请和释放内存使用的是malloc()/free()等函数。内存池技术的思想非常简单，通俗地说就是“一次批发，多次零售”。malloc()和free()是系统调用，其开销相对较大，所以为了提高软件的性能，就要尽量减少对malloc()和free()的调用次数。因为用malloc()申请小块的内存和申请大块内存的开销并无多大差别，所以为什么不一次性申请一大块内存，再慢慢切蛋糕享用呢？对，这就是内存池技术的核心思想。内存池技术一次性使用malloc()分配大块内存，类似批发；后来的小额内存申请会从这个大块内存中不断切出来尺寸合适的内存片，类似零售。而这种切蛋糕的操作只是简单的内存指针的位置移动，不再涉及系统调用，所以非常高效。当这块大内存上所有的内存都不再使用后，再一次性通过free()再把这块内存释放掉。内存池的这个基本思想是非常简单易懂的。
...
在PostgreSQL的缺省实现策略中，每当通过malloc()分配一个新的Block时，它的体积都比前一次分配的Block大一倍。例如：第一个初始Block的大小是8KB，则第二次分配的Block的大小是16KB，再下一次是32KB，再下一次是64KB，依次类推，直至新Block的体积等于maxBlockSize，就不再增加了，以后就按照maxBlockSize大小分配新的Block。内存池中的所有Block组成一个双向链表，有头有尾。我们从上图中可以看到，1号，2号，3号，4号块依次加倍增加，且它们形成了双向链表，其中4号块为首块，1号块为尾块。

内存池中还有一个概念：活跃Block(Active Block)。上图中最左边的4号Block便是活跃Block。当申请者使用palloc()函数向内存池申请内存时，内存池返回给申请者的内存叫内存片(Chunk)，Chunk是从内存池中某个Block中切下来的一小块。内存池为Chunk寻找空闲内存的策略是：内存池只在活跃Block中的空闲内存区中寻找。为什么只能在活跃Block中寻找空闲内存呢？双向链表上其它的块就没有空闲内存了吗？其它块真的没有空闲空间了。原因如下：在内存池创建伊始，只有一个初始Block，所以初始Block是内存池初期的活跃Block。只有当目前的活跃Block中的空闲内存不够时，内存池才会再次malloc()出一个更大的新块，并且把它放在Block双向链表的头部，作为新的活跃Block。活跃Block是在其它老的Block中没有空闲内存的时候才会诞生，故而内存池中只有活跃Block才可能存在空闲内存可供未来的分配，这就是“活跃”的含义。活跃块永远是双向链表中的第一个块。内存池只在活跃Block里面寻找空闲内存，不用费力地遍历双向链表，这提高了搜索空闲内存的效率。Block组成的双链只是用于内存池销毁或者重置的时候进行遍历使用。
