...
PostgreSQL内核基本上都是采用C语言编写的，而且目前只支持C99标准，目标就是能够让PostgreSQL运行在更加广泛的硬件平台和操作系统之上。C语言作为排名第一的系统软件开发语言，它给了程序员最大的内存管理的自由度，也带来了很多潜在的问题。为了避免C语言编写的软件出现内存泄漏，高质量的软件往往采用内存池的技术，PostgreSQL也不例外。本章会带领读者剖析PostgreSQL内存池子系统的设计思想和具体实现细节。
