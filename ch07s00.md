...
C语言初学者都知道，在C程序中申请和释放内存使用的是malloc()/free()等函数。内存池技术的思想非常简单，通俗地说就是“一次批发，多次零售”。malloc()和free()是系统调用，其开销相对较大，所以为了提高软件的性能，就要尽量减少对malloc()和free()的调用次数。因为malloc(1)和malloc(8192)的开销差别不大，所以内存池技术会一次性使用malloc()分配大块内存，类似批发；后来的小额内存申请会从这个大块内存中不断切出来尺寸合适的内存片，类似零售。而这种切蛋糕的操作只是简单的内存指针的位置移动，不再涉及系统调用，所以非常高效。当这块大内存上所有的内存都不再使用后，再一次性通过free()再把这块内存释放掉。内存池的这个基本思想是非常容易简单易懂的。

## 内存池子系统概述

我们可以把PostgreSQL的内存池理解为一堆内存块(Block)的集合。这些Block是通过malloc()分配的，最终通过free()来进行释放，且它们组成一个双向链表。内存池管理子系统对外提供了palloc()和pfree()的内存分配和释放接口函数，来代替操作系统的malloc()和free()等系统调用。所以在PostgreSQL中，当你需要申请内存的时候，不是使用malloc()，而是使用palloc()；释放内存的时候不是使用free()，而是使用pfree()。PostgreSQL的内存池概念可以用下图表示：
...
内存池中还有一个概念：活跃Block。当申请者使用palloc()函数向内存池申请内存时，内存池返回给申请者的内存叫内存片(Chunk)，Chunk是从内存池中某个Block中切下来的一小块。内存池为Chunk寻找空闲内存的策略是：内存池只在活跃Block中的空闲内存区中寻找。很显然，在内存池创建伊始，只有一个初始Block，所以初始Block是内存池初期的活跃Block。只有当目前的活跃Block中的空闲内存不够时，内存池才会再次malloc()出一个更大的新块，并且把它放在Block双向链表的头部，这个Block被称为“活跃Block”(Active Block)。活跃Block是在其它老的Block中没有空闲内存的时候才会诞生，故而内存池中只有活跃Block才可能存在空闲内存。内存池只在活跃Block里面寻找空闲内存，不用费力地遍历双向链表，这就是“活跃”的含义。上图中最左边的Block便是活跃Block。Block组成的双链只是用于内存池销毁或者重置的时候进行遍历使用。
