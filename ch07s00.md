...
PostgreSQL内核是采用C语言编写的，且目前只支持C99标准。C语言作为排名第一的系统软件开发语言，它给了程序员最大的内存管理的自由度，也带来了很多潜在的问题。为了避免C语言编写的软件出现内存泄漏，高质量的软件往往采用内存池的技术，PostgreSQL也不例外。本章会带领读者剖析PostgreSQL内存池子系统的设计思想和具体实现细节。

注意：本章相对比较独立。建议读者在第一次阅读本书时，可跳过本章。当你需要对PostgreSQL源代码普遍存在的palloc()/pfree()等本地内存管理函数的实现有更深入理解的时候，可再回过头来阅读本章的内容。

C语言初学者都知道，在C程序中申请和释放内存使用的是malloc()/free()等函数。内存池技术的思想非常简单，通俗地说就是“一次批发，多次零售”。malloc()和free()是系统调用，其开销相对较大，所以为了提高软件的性能，就要尽量减少对malloc()和free()的调用次数。因为malloc(1)和malloc(8192)的开销差别不大，所以内存池技术会一次性使用malloc()分配大块内存，类似批发；后来的小额内存申请会从这个大块内存中不断切出来尺寸合适的内存片，类似零售。而这种切蛋糕的操作只是简单的内存指针的位置移动，不再涉及系统调用，所以非常高效。当这块大内存上所有的内存都不再使用后，再一次性通过free()再把这块内存释放掉。内存池的这个基本思想是非常简单易懂的。
...
在上图中，最右边的1号Block是内存池刚刚创建时用malloc()分配的初始Block(Initial Block)。它的大小由一个整型变量initBlockSize控制，一般为8192，所以初始Block的大小通常为8K。内存池使用三个变量来控制每次分配的Block的大小，它们的单位均为字节(in bytes)：
- initBlockSize - 初始Block的大小。
- nextBlockSize - 下一次分配的Block的大小。
- maxBlockSize  - 最大Block的大小。

在PostgreSQL的缺省实现策略中，每当通过malloc()分配一个新的Block时，它的体积都比前一次分配的Block大一倍。例如：第一个初始Block的大小是8KB，则第二次分配的Block的大小是16KB，再下一次是32KB，再下一次是64KB，依次类推，直至新Block的体积等于maxBlockSize，就不再增加了，以后就按照maxBlockSize大小分配新的Block。内存池中的所有Block组成一个双向链表，有头有尾。

内存池中还有一个概念：活跃Block(Active Block)。上图中最左边的4号Block便是活跃Block。当申请者使用palloc()函数向内存池申请内存时，内存池返回给申请者的内存叫内存片(Chunk)，Chunk是从内存池中某个Block中切下来的一小块。内存池为Chunk寻找空闲内存的策略是：内存池只在活跃Block中的空闲内存区中寻找。很显然，在内存池创建伊始，只有一个初始Block，所以初始Block是内存池初期的活跃Block。只有当目前的活跃Block中的空闲内存不够时，内存池才会再次malloc()出一个更大的新块，并且把它放在Block双向链表的头部，作为新的活跃Block。这个Block就被称为“活跃Block”。活跃Block是在其它老的Block中没有空闲内存的时候才会诞生，故而内存池中只有活跃Block才可能存在空闲内存。内存池只在活跃Block里面寻找空闲内存，不用费力地遍历双向链表，这就是“活跃”的含义。Block组成的双链只是用于内存池销毁或者重置的时候进行遍历使用。
...
在内存池的控制头AllocSetContext中，有一个数组叫freelist，它的成员是指向Chunk的指针。freelist保留着通过pfree()释放回本内存池的内存片Chunk，而这些空闲的Chunk可以废物再利用，被下次的palloc()使用。在上图中读者可以看到freelist的大致形态。关于freelist，我们后面会详细讨论。
...
在PostgreSQL的内存池子系统中有几个数据结构：MemoryContext(内存上下文)，AllocSetContext(分配集合上下文)，AllocBlock(管理内存块Block的结构体)和AllocChunk(管理内存片Chunk的结构体)。下面介绍它们的定义，含义和之间的关系。

### 内存上下文MemoryContext

MemoryContext是指向一个结构体MemoryContextData的指针，它们的定义列在下面。在后面的讨论中，为了减少单词的长度，本书把结构体和指向该结构体的指针大致视为同一个概念，可能会交叉使用。后面也有相似的例子，不再赘述。MemoryContext是内存池的一种抽象的控制头，具体对内存池的创建、内存分配和释放、内存池的销毁等工作由具体的控制头来实现。这个类似C++/Java等面向对象程序设计语言中的父类和子类的关系。在PostgreSQL中由AllocSetContext来具体实现MemoryContext。
```c
...
我们观察到MemoryContext有几个指针，parent/firstchild/prevchild/nextchild，通过它们，多个内存池可以形成了一个树形结构，如下图所示，其中每一个矩形都表示一个内存池。

![](d0024.svg)

我窃以为：prevchild和nextchild变量的名字起的不好。它们实际上指向了自己的兄弟，所以应该叫做pre_sibling和next_sibling。

在PostgreSQL中只有一个TopMemoryContext，它是处于根节点位置的内存池，它的生命周期贯穿整个PostgreSQL实例：从PostgreSQL实例开始运行到结束为止。在它下面还有若干内存池。它们使用如下的全局变量来进行记录：
```c
