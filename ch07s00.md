...
在PostgreSQL的缺省实现策略中，采用贪吃型的内存分配方案：每当通过malloc()分配一个新的Block时，它的体积都比前一次分配的Block大一倍。例如：第一个初始Block的大小是8KB，则第二次分配的Block的大小是16KB，再下一次是32KB，再下一次是64KB，依次类推，直至新Block的体积等于maxBlockSize，就不再增加了，以后就按照maxBlockSize大小分配新的Block。这些Block组成一个双向链表，有头有尾。每次malloc()分配一个更大的Block之后，内存池都会把这个新鲜出炉的Block放在Block双向链表的头部。我们把这个Block叫做“Active Block”，即活跃Block。由于内存池每次分配内存片的时候，是在活跃Block上的空闲内存中寻找合适大小的，只有活跃Block上的空闲内存不够了，才会再次malloc()出一个更大的新块，所以活跃Block后面的老的Block中基本上没有什么空闲内存了。内存池在寻找空闲内存的时候，肯定是在活跃Block里面找，不用遍历这个双向链表了。这是“活跃”的基本含义。上图中最左边的Block便是活跃Block。Block组成的双链只是用于内存池销毁或者重置的时候进行遍历使用。

初始Block和其后诞生的Block有一点不同：在初始Block的头部有个“控制头”AllocSetContext，这是一个C语言的结构体，其中的成员变量用于控制整个内存池的分配销毁等行为。上图中的右下角的大矩形就是AllocSetContext。请注意：它实际上是初始Block头部很小的一块区域，只是为了显示其细节，图中进行了放大。正因为初始Block和其后诞生的Block的头部有这个不同，所以产生了内存池的删除(Delete)和重置(Reset)两种操作的概念，具体含义稍后讨论。AllocSetContext中有一个指针blocks，始终指向内存池中Block双向链表的活跃Block。
