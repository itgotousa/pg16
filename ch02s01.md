...
在前一章介绍PostgreSQL体系结构时，我们知道共享内存中最大的组件是shared buffer池。它里面就是一个个读入到内存中的Page。我们可以把shared buffer池理解为一个数组，每个成员都是8192个字节。shared buffer池在PostgreSQL主进程启动后就固定分配下来了，它的尺寸在PostgreSQL实例运行期间是不能改变的。它的尺寸由一个全局变量NBuffers规定。我们可以用如下的C语言代码来表示shared buffer池的结构：
```c
typedef struct {
    char context[8192];
} PageData;
...
PageData page[NBuffers]

Page p = page;
for(int i = 0; i < NBuffers; i++) {
    /* do something on p[i] here */
}
```
