...
Block和Page是PostgreSQL世界里面经常互相替换使用的两个术语。我们已经知道：PostgreSQL的数据文件是按照Block划分的，当磁盘上的一个Block被读入到内存中后，内存中也会分配8K的空间来保存它，称之为Page（页）。简而言之：在磁盘上是Block，在内存中则为Page，两者的内容是一模一样的。下面我们就来分析Block或者Page的基本结构。
