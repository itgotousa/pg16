...
毫无疑问，数据文件是数据库中体积最大，也是最重要的文件之一。数据在数据文件中可以按行存储，也可以按列存储，由此产生了行式数据库（Row Oriented Databases）和列式数据库（Columnar Database）两种类型。传统的关系型数据库，包括Oracle,Microsoft SQL Server, MySQL和PostgreSQL都是行式数据库。本节我们来对PostgreSQL的数据文件的结构进行研究。

### 数据文件的基本结构

在PostgreSQL中，每一张表对应一个数据文件。最大的表可以有32T。很显然，这么巨大的文件不便于管理。而且操作系统对于文件的尺寸有限制，譬如在32位的操作系统中，文件大小最大不能超过4G。为了管理PostgreSQL的表，我们可以把对应于一个表的文件设想为“虚拟文件”。这个文件可以切分成很多小的物理文件。这些物理文件被称为这个虚拟文件的“段”(segment)。如下图所示：

![](d0063.svg)

在缺省情况下，PostgreSQL把数据文件的segment的最大尺寸设置为1G。这样的文件就比较好管理了。其中每一个segment，又被切分成了块(Block)。在PostgreSQL的源代码中有两个常量，BLCKSZ规定Block的大小，单位是字节。RELSEG_SIZE规定数据文件segment的大小，单位是块。其缺省定义如下：
```c
...
```
我们可以看出，在编译源代码的时候，可以使用上边两个选项来改变缺省的设置。
