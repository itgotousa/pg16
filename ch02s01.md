...
在缺省情况下，PostgreSQL把数据文件的segment的最大尺寸设置为1G，这个尺寸的文件就比较好管理了。其中每一个segment，又被切分成了块(Block)。在PostgreSQL的源代码中有两个常量，BLCKSZ规定Block的大小，单位是字节；RELSEG_SIZE规定数据文件segment的大小，单位是块。其缺省定义如下：
```c
...
```
所以为了改变Block和Segment的大小，我们在编译源代码的时候，可以使用上边两个选项。缺省的8192和1G对于绝大多数情况下都是非常理想的设置，所以一般情况下我们没有必要去改变这两个参数的设置。
