...
```
如果你编写一个小程序测试一下，就会发现由于对齐的原因，XLogRecord实际上占有24个字节。
数据部分CheckPoint的数据结构定义如下：
```c
...
```
从上面的实验，我们可以看出，当我们手工执行CHECKPOINT命令完毕后，控制文件里面的CheckPoint和REDO Point的LSN都发生了前移的现象。目前的检查点的LSN是0/33ED230，且当前检查点是1，根据前面我们我们学习的知识，可以推知：
...
```
根据上面的结果，我们可以分析出来：
- XLogRecord.xl_tot_len = 114 (0x72)
- XLogRecord.xl_xid = 0
