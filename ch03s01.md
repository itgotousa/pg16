...
我们可以看到，一个检查点的执行大体上分为五个主要的动作：
- 步骤1是CheckPoint发生的开始，此时Checkpointer进程会首先记录当前的WAL指针的位置，称为REDO point。
- 步骤2开始在shared buffer池中寻找脏页，依次把这些脏页写入到磁盘的数据文件中。
- 步骤3表示把脏页刷新到数据文件的过程。这个过程可能很长，持续几十分钟都不罕见。当所有的脏页被写入到数据文件以后，才能进入到了步骤4。
- 步骤4的动作是往WAL文件中插入一个CheckPoint的WAL记录。后面我们会详细分析该WAL记录的细节。
- 步骤5是把步骤4中的CheckPoint信息写入到控制文件。
...
void CreateCheckPoint(int flags)
{
    ...
    ...
    checkPoint.redo = curInsert;   /* 步骤1 - 把当前WAL的位置记录在redo中 */
    ...
    /* 步骤2/3 - CheckPointGuts函数是写脏页到数据文件中。它的执行时间最长 */
    CheckPointGuts(checkPoint.redo, flags); 
    ...
    XLogBeginInsert(); /* 步骤4 - 把CheckPoint WAL记录写入WAL文件中 */
    XLogRegisterData((char *) (&checkPoint), sizeof(checkPoint));
    recptr = XLogInsert(RM_XLOG_ID, shutdown ? XLOG_CHECKPOINT_SHUTDOWN : XLOG_CHECKPOINT_ONLINE);

    XLogFlush(recptr);
    ...
    UpdateControlFile();  /* 步骤5 - 更新控制文件 */
    ...
}
```
这里面有一个关键的概念：重做点(REDO point)。Redo point是执行数据库恢复的起点，因为在它之前的所有WAL记录都不再被当前数据库需要了。为什么呢？因为REDO point是从步骤4或者步骤5中拿到的，既然我们拿到了REDO point，则证明步骤4和5已经执行成功了。这也意味着步骤3执行成功了，即：所有的脏页都被可靠地写入了磁盘上的数据文件。在REDO point之前的WAL记录描述了脏页的变化，现在脏页已经不脏了，所以REDO point之前的WAL记录对于当前数据库就没有用处了。为了恢复当前数据库，必须要把REDO point作为起点，它右边的WAL记录对当前数据库的恢复才具有意义。如果步骤3失败了，会怎么样呢？步骤3失败了，则后面的步骤4和5页不会成功，则控制文件里面的内容不会被更新，控制文件依然记录着前一次成功地Checkpoint的位置。那么数据库恢复就从上一个REDO point开始，本次的REDO point就没有意义了。所以PostgreSQL启动的时候，会查看控制文件里面的检查点和REDO point信息，把控制文件里面记录的REDO point作为恢复的起点，请参考下图：

![](d0034.svg)

由于Checkpoint可能会执行很长时间，所以一个Checkpoint的WAL记录中的redo，可以理解为该Checkpoint的起点，它的WAL记录作为终点。
...
图中的三部分的尺寸都是以PostgreSQL 15.2为准，后面的版本可能会稍微有一些不同。这三个结构的具体定义列在下面。

记录头XLogRecord的数据结构如下：
```c
...
typedef struct XLogRecord {
    uint32          xl_tot_len;  /* 整个记录的长度(in bytes) */
    TransactionId   xl_xid;        /* xact id */
    XLogRecPtr      xl_prev;     /* 指向前一个WAL Record的位置(LSN) */
    uint8           xl_info;     /* flag bits, see below */
    RmgrId          xl_rmid;     /* resource manager for this record */
    /* 2 bytes of padding here, initialize to zero */
    pg_crc32c       xl_crc;      /* 本记录的校验码CRC */
} XLogRecord;
```
如果你编写一个小程序测试一下，就会发现由于对齐的原因，XLogRecord实际上占有24个字节。 数据部分CheckPoint的数据结构定义如下：
```c
