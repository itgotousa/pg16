...
```
这里面有一个关键的概念：重做点(REDO point)。Redo point是执行数据库恢复的起点，因为在它之前的所有WAL记录都不再被当前数据库需要了。为什么呢？因为REDO point是从步骤4或者步骤5中拿到了。既然我们拿到了REDO point，则证明步骤4和5已经执行成功了，也意味着步骤3执行成功了，即：所有的脏页都被可靠地写入了磁盘上的数据文件。在REDO point之前的WAL记录描述了脏页的变化，现在脏页已经不脏了，所以REDO point之前的WAL记录对于当前数据库就没有用处了。为了恢复当前数据库，必须要把REDO point作为起点，它右边的WAL记录对当前数据库的恢复才具有意义。由于Checkpoint可能会执行很长时间，所以一个Checkpoint的WAL记录中的redo，可以理解为该Checkpoint的起点，它的WAL记录作为终点。
