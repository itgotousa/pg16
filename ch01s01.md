...
程序是一个躺在磁盘上的文件，如果它不被运行，就没有多大用处。只有用户运行它后，它才能够为我们提供应有的服务。我们使用pg_ctl start命令运行PostgreSQL程序。被运行起来的程序被称为“进程”(process)。譬如我们每打开一个我们熟悉的putty窗口，Windows操作系统就创建了一个进程。我们可以打开多个Putty窗口，就是启动了多个进程。这些Putty的进程可以在Windows下面的Task Manager里面被查看到。在Linux等类Unix操作系统中查看进程的主要命令就是ps。我们在上节的实验中已经使用过了。
...
关于进程线程的概念和关系的进一步探讨，涉及到操作系统等深入的知识。我们不在这里做过多的展开。这两个概念是非常核心的概念。你可以在互联网上搜索相关资料进一步研究。
