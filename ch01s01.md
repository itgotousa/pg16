...
程序是一个躺在磁盘上的文件，如果它不被运行，就没有多大用处。只有用户运行它后，它才能够为我们提供应有的服务，例如我们使用pg_ctl start命令运行PostgreSQL程序。被运行起来的程序被称为“进程”(process)。譬如我们运行的每一个putty窗口，就是一个进程。我们可以运行多个Putty窗口，就是多个进程。它们可以在Windows下面的Task Manager里面被查看到。在Linux等类Unix操作系统中查看进程的主要命令就是ps。我们在上节的实验中已经使用过了。
