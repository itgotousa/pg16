...
学习任何一个新东西，第一件事情就是要对它有一个整体印象。本章是第一章，自然要介绍PostgreSQL的体系结构。但是在这之前，当务之急是要手头有一个可以做各种实验的环境。所以我们在第一节要先介绍如何搭建PostgreSQL的学习实验环境。学习PostgreSQL，如果不去阅读源码，理解其背后的技术秘密，就是辜负了“开源”两个字。故本章只给大家讲解如何从源代码开始安装PostgreSQL，既学习PostgreSQL的外在软件功能，又探究其背后的源码实现和技术秘密。努力做到“知其然且知其所以然”是本书最大的特色之一。
