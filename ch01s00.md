...
我们知道，在Linux环境下的软件，往往需要设置一些环境变量和配合软件的运行，如Oracle软件需要设置ORACLE_HOME和ORACLE_SID等环境变量。PostgreSQL常用的环境变量只有两个：PGHOME和PGDATA。它们的含义如下：
- PGHOME指向PostgreSQL的软件安装目录，在我们这里是/opt/software/pg152。然后你把$PGHOME/bin加入到PATH环境变量当中以后，就可以在任何地方使用PostgreSQL的各种命令了。否则每次你都要输入类似/opt/software/pg152/pg_ctl的命令，前面带着一个常常的绝对路径，非常繁琐。
