...
本书的全部实验都是在Linux环境下进行的，不讨论Windows环境。故而本书要求读者具备一定的Linux使用经验，且为了能够理解PostgreSQL的源代码，读者还需要具备初步的C语言编程的能力。如果你不具备Linux基本操作和C语言编程的能力，也不要担心，因为本书需要的Linux和C语言的知识是非常少的。对于Linux的操作部分，只要求掌握ps, ls, cd, pwd, vi等常用的命令，对于C语言，只要求知道0xA57D是16进制，理解struct的数据结构，指针的基本使用等。所以你可以在互联网上寻找相关资料进行快速学习。这方面的文档和视频等学习资料是非常丰富的。你只需恶补一两个星期左右，所掌握的Linux和C编程的知识就足够学习本书的内容了。
