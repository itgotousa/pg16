...
try_it_again:
    if((*lock) == 0) { /* 如果lock里面的值为0， 表示无人使用这把锁 */
        *lock = 1; /* 加锁！我来使用这把锁！*/
    } else goto try_it_again; /* 否则就不停地循环检测lock被别人释放了 */
}
...
```
上述代码非常浅显易懂。但是里面存在一个致命的问题。在get_spin_lock()函数中，if((*lock) ==0)是检查自旋锁lock的状态，是一条测试指令(TEST)。而*lock = 1是往自旋锁里面放1，是一条赋值指令(SET)。在TEST指令和SET指令之间，有可能被中断。也就是说刚刚检测到自旋锁的状态为0，打算自己占这个坑的时候，有可能别人捷足先登，在TEST和SET指令之间就先执行了一条SET指令，把lock占住了，而我们毫无察觉。为了保证可靠地获取自旋锁，必须保证TEST指令和SET指令具有原子性，即它们不能被中断。我们都知道，一条指令的执行是不能够被中断的。如果是多核CPU或多个CPU的情况下，可以使用LOCK前缀独占指令总线和数据总线，从而保证一条指令是原子性的，这条指令在执行过程中不可能被打断。因此我们需要硬件提供一条TEST and SET的指令，在一条指令中完成检测和赋值两个任务。这是自旋锁可靠性的根本保证。这样的指令被称为TAS(Test and Set)指令。
