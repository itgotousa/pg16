...
我们先演示一下两个简单的C程序。第一个例子研究了父进程中通过malloc()创建了一块私有内存，然后通过fork()创建了一个子进程，那么子进程和父进程创建的私有内存的关系？请看源代码：
```c
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
pid_t pid;
    ...
    printf("Hello, this is the testing for fork() and malloc() !\n");
...
下面是该程序运行的结果：
```
$ gcc -Wall f1.c -o f1 /* 编译该源程序，产生了一个可执行文件f1 */
$ ./f1    /* 执行f1程序 */
Hello, this is the testing for fork() and malloc() !
Hello, this is the testing for fork() and malloc() !
...
char *p;
	...
	printf("Hello, this is the testing for fork() and mmap()!\n");
