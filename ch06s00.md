...
pid_t fork(void);
```
从外表上看，fork()的定义非常简单，就是一个没有任何输入参数的普通函数。实际上它是一个重要的Linux系统调用。一个进程通过它来创建子进程。fork()返回一个pid_t类型的值，就是一个无符号整数。当它返回的值是-1(0xFFFFFFFF)时，表示创建子进程失败。当它返回的值为0的时候，表示下面的代码将在子进程中运行。当它返回的值大于0，则表示下面的代码依然在本进程中运行。所以fork()的基本使用是这样的：
```c
pid_t  result;
