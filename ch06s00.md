...
```
根据上述S_LOCK(lock)的定义，我们可以看出：它首先调用了TAS(lock)，如果TAS(lock)返回0，则表明此时没有人使用lock这把锁，TAS就会把lock里面的内容变成1，表明既然大家目前都不在使用这把锁，那么本人就不客气了，直接上锁，走人。此时S_LOCK(lock)立刻返回0。如果TAS(lock)返回非0，则表明无法获得这把锁，就开始调用slock()函数。下面我们研究一下TAS(lock)和slock(lock)的具体细节。先看TAS(lock)，上源码：
```c
/* in src/include/storage/s_lock.h */
